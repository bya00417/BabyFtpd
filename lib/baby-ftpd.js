"use strict";
var BabyFtpd, exec, fs, mkdirp, net, path, winston;
...
module.exports = BabyFtpd = (function() {
  var authUser, commands, dtpServer, logger, messages;
  ...
  function BabyFtpd(option) {
    ...
    this.fileSystem = new BabyFtpd.FileSystem(logger);
  ...
  BabyFtpd.prototype.listen = function(port, host) {
    ...
    this.piServer.on('listening', function() {
      ...
      return logger.log("info", "Server listening on " + hostInfo.address + ":" + hostInfo.port);
    });
    this.piServer.on('connection', function(socket) {
      ...
      socket.on('close', function() {
        return logger.log("info", "Server socket closed.");
      });
  ...
  BabyFtpd.changeCarriageCode = function(data) {
    var retData;
      ...
      return this.fileSystem.getNlst(this.sessionDir, reqPath, (function(_this) {
        return function(err, files) {
          ...
          if (err != null) {
            ...
            return _this.dtpServer.dtpSocket.sender(ls);
          }
        };
      })(this));
    },
          ...
          return this.dtpServer.store((function(_this) {
            return function(storeData) {
              return _this.fileSystem.setFile(storeData, _this.sessionDir, reqPath, function(err) {
                if (err != null) {
                  if (err.message === "Disk quota exceeded") {
                    return _this.reply(552, "Transfer aborted. Disk quota exceeded");
                  } else {
  ...
  dtpServer = function(socket) {
    ...
    dtp.on("connection", function(dtpSocket) {
      ...
      dtpSocket.on("close", function() {
        return logger.log("info", "DTP Socket closed");
      });
      dtpSocket.on("connect", function() {
        return logger.log("info", "DTP Socket connect");
      });
...
BabyFtpd.FileSystem = (function() {
  ...
  FileSystem.prototype.setBase = function(dirPath) {
    ...
    try {
      return fs.statSync(this.baseDir);
    } catch (_error) {
      return mkdirp.sync(this.baseDir);
    }
  };

  FileSystem.prototype.setQuotaSize = function(qSize) {
    return this.quotaSize = qSize;
  };
  ...
  FileSystem.prototype.setFile = function(storeData, nowDir, reqPath, callback) {
    var srorePath;
    if (this.quotaSize > 0 && storeData.length > this.quotaSize) {
      logger.log("info", "Disk quota exceeded");
      return callback(new Error("Disk quota exceeded"));
    } else {
